...
class CNTextInput extends Component {
  ...
  handleChangeText = text => {
    ...
    if (textDiff >= 0) {
      ...
      if (foundItemNo >= textDiff) {
        ...
        content[foundIndex].len -= textDiff;
        content[foundIndex].text =
          txt.substring(0, foundItemNo - textDiff) +
  ...
  addToUpComming(toolType) {
    if (this.upComingStype) {
      ...
      let newUpStype = this.upComingStype
        ? indexOfUpToolType != -1
  ...
  applyStyle(toolType) {
    ...
    for (var i = 0; i < content.length; i++) {
      ...
      let newStyles = StyleSheet.flatten(
