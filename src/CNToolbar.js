...
class CNToolbar extends Component {
  ...
  componentDidMount() {
    if (!this.props.iconSet)
      console.warn(
        "CNToolbar requires `iconSet` prop to display icons (>= 1.0.41). Please check documentation on github."
      );
    if (
      this.props.bold ||
      this.props.italic ||
      ...
      console.warn(
        "CNToolbar: using `bold`, `italic`, `underline`, `lineThrough`, `body`, `title`, `heading`, `ul`, `ol`, `image`, `highlight` or `foreColor` is deprecated. You may use `iconSet` prop instead (>= 1.0.41)"
      );
    }
  }
      ...
      <View style={[styles.toolbarContainer, style]}>
        {iconSet.map((object, index) => {
          return object.type !== "seperator" &&
            ...
            <CNSeperator key={index} color={color || defaultColor} />
          );
        })}
      </View>
    );
